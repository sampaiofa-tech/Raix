# Raix Codebase Consolidada — Módulo 3: Governança, Threat Model & Pipelines de CI/CD
...
### Arquivo: `AGENTS.md`
**Contexto Arquitetural:** Diretrizes de Operação do Agente de IA — Governança permanente de segurança e isolamento de credenciais.
...
### Arquivo: `docs/THREAT_MODEL.md`
**Contexto Arquitetural:** Modelo de Ameaça e Postura de Segurança Pós-Quântica — Detalhamento de vetores, combiners híbridos e análise de tráfego por IA.
...
| **SSL Stripping, MitM de Borda e Spoofing de E-mail (P2 & P3)** | Ataques de downgrade para HTTP não cifrado e envio de mensagens fraudulentas em nome do canal institucional `contato@raixtech.com`. | **HSTS Mandatório na Borda (`max-age=31536000; includeSubDomains; preload`)** em Cloudflare e GitHub Pages, aliado à proteção anti-phishing tríplice: **SPF (`include:_spf.google.com ~all`)**, **DKIM (RSA 2048-bit `google._domainkey`)**, **DMARC (`_dmarc.raixtech.com`)** e **DNSSEC ativo**. Regras de WAF e Super Bot Fight Mode configuradas no Cloudflare (ativação de proxy como hardening pós-reunião, preservando DNS-only pré-reunião para estabilidade do GitHub Pages). Domínio adicional `raixtech.com.br` configurado como redirecionamento 301 para `https://raixtech.com` (SSL em propagação no Registro.br/Cloudflare; sem serviços de e-mail associados, dispensando SPF/DKIM/DMARC no momento; higiene DNS verificada sem CNAMEs órfãos). Detalhes em `docs/DNS_SECURITY.md`. |
...
### 12.1 Conclusão da Auditoria de Sanitização (Commit `b5b57a1`)
 ...
 - **Perímetro de Domínios e Redirecionamento .com.br**: Domínio nacional adicional `raixtech.com.br` auditado em 07/09/2026: configurado como redirecionamento 301 para `https://raixtech.com`; status SSL reportado como pendente de propagação (Registro.br ➔ Cloudflare); decisão registrada de que não é crítico configurar SPF/DKIM/DMARC por ausência de serviços de e-mail associados; higiene de DNS validada sem CNAMEs pendurados/órfãos e suporte a DNSSEC confirmado no Registro.br.
...
**Contexto Arquitetural:** Pipeline principal de CI — Verificação de integridade, testes unitários e validação de regras.
...
on:
  schedule:
    # 07:00 UTC (04:00 Horário de Brasília / UTC-3)
    - cron: '0 7 * * *'
    # 23:00 UTC (20:00 Horário de Brasília / UTC-3)
    - cron: '0 23 * * *'
  workflow_dispatch:
...
jobs:
  health-check:
    name: Regression Suite & Health Monitor
    ...
    steps:
      ...
      - name: Setup Java 21 (Required for Gradle & Firestore Emulator)
      ...
      - name: Restore Last Health Check State Cache
        uses: actions/cache/restore@v4
        id: last-state-cache
        with:
          path: |
            .health_check_last_commit
            .health_check_consecutive_failures
          key: health-check-state-${{ runner.os }}-
          restore-keys: |
            health-check-state-${{ runner.os }}-
      ...
      - name: Run Backend Tests (Cloud Functions)
        id: test_backend
        continue-on-error: true
        run: npm --prefix functions test

      - name: Run Firestore Index Coverage Check
        id: test_indexes
        continue-on-error: true
        run: npm --prefix functions run check:indexes

      - name: Run Desktop Tests (Compose Desktop)
        id: test_desktop
        continue-on-error: true
        run: ./gradlew :composeApp:desktopTest --no-daemon --stacktrace

      - name: Run Android Unit Tests (Compose Android)
        id: test_android
        continue-on-error: true
        run: ./gradlew :composeApp:testDebugUnitTest --no-daemon --stacktrace

      - name: Run Integrity Audit (Artifact Hashes, SBOM & Config)
        id: test_integrity
        continue-on-error: true
        run: node scripts/verify_integrity.mjs

      - name: Run Vulnerability Scan (CVEs - npm audit)
        id: test_cve
        continue-on-error: true
        run: npm audit --prefix functions --audit-level=high

      - name: Run Secret Leak Scan (verify_secrets)
        id: test_secrets
        continue-on-error: true
        run: node scripts/verify_secrets.cjs

      - name: Run Firestore Adversarial Rules (Emulator Suite)
        id: test_rules_emulator
        continue-on-error: true
        run: npx -y firebase-tools@latest emulators:exec --only firestore "npm --prefix functions run test:emulator"

      - name: Run Hosting & Network Latency Monitor
        id: test_hosting
        continue-on-error: true
        run: node scripts/monitor_hosting.mjs

      - name: Generate Consolidated Health Check Reports (HTML & TXT)
        id: report
        if: always()
        run: |
      ...
      - name: Send Health Check Email Report
        if: always()
        ...
        with:
          ...
          subject: ${{ steps.report.outputs.email_subject }}
          priority: ${{ steps.report.outputs.email_priority }}
          to: ${{ secrets.SMTP_TO }}
          from: "Pmsg Health Monitor <${{ secrets.SMTP_USER }}>"
          html_body: file://report.html
          body: file://report.txt

      - name: Prepare State for Cache
        if: always()
        run: |
          echo "${{ github.sha }}" > .health_check_last_commit
          echo "${{ steps.report.outputs.consecutive_failures }}" > .health_check_consecutive_failures

      - name: Save Health Check State Cache
        if: always()
        ...
        with:
          path: |
            .health_check_last_commit
            .health_check_consecutive_failures
          key: health-check-state-${{ runner.os }}-${{ github.run_id }}

      - name: Enforce Overall Job Status
        if: always()
        run: |
          if [ "${{ steps.report.outputs.overall_status }}" != "SUCCESS" ]; then
            echo "❌ Uma ou mais suítes de testes de regressão falharam. Finalizando job com código de erro."
            exit 1
          fi
          echo "✅ Todas as suítes de teste foram executadas com sucesso."
```

---

### Arquivo: `.github/workflows/health-check-watchdog.yml`
**Contexto Arquitetural:** Watchdog de integridade contínua e monitoramento de dependências.

```yaml
name: Health Check Watchdog (Missed Execution Monitor)

on:
  schedule:
    # Executa 2x ao dia às 12:00 UTC e 00:00 UTC para auditar os ciclos de 07:00 e 23:00 UTC
    - cron: '0 0,12 * * *'
  workflow_dispatch:
...
jobs:
  watchdog:
    name: Missed Execution Watchdog
    runs-on: ubuntu-latest
    timeout-minutes: 10

    steps:
      - name: Audit Health Check Execution Recency
        id: check_health
        run: |
      ...
      - name: Send Missed Execution Alert Email
        if: steps.check_health.outputs.is_missed == 'true'
        uses: dawidd6/action-send-mail@v3
        with:
          server_address: ${{ secrets.SMTP_HOST }}
          server_port: ${{ secrets.SMTP_PORT }}
          ...
          password: ${{ secrets.SMTP_PASS }}
          priority: 'high'
          subject: "🚨 [Pmsg Watchdog] ALERTA CRÍTICO: Health-Check Inativo (> 24h sem execução)"
          to: ${{ secrets.SMTP_TO }}
          from: "Pmsg Watchdog <${{ secrets.SMTP_USER }}>"
          html_body: file://watchdog_report.html
          body: file://watchdog_report.txt

      - name: Enforce Watchdog Assertion
        if: steps.check_health.outputs.is_missed == 'true'
        run: |
          echo "❌ FALHA: Health-check não rodou nas últimas 24h (${{ steps.check_health.outputs.diff_hours }}h decorridas). Notificação enviada."
          exit 1
```

---

### Arquivo: `.github/workflows/supply-chain-attestation.yml`
**Contexto Arquitetural:** Automação de Supply Chain — Geração de SBOM CycloneDX v1.5 e atestado de proveniência.

```yaml
name: Supply Chain Integrity & SLSA Attestation (P0.4)
...
jobs:
  verify-dependencies:
    name: Verify Pinned Dependencies & Hashes
    runs-on: ubuntu-latest
    steps:
      - name: Checkout Repository
        uses: actions/checkout@v4
      ...
      - name: Verify Functions Hashes (npm ci)
      ...
      - name: Cache Gradle Dependencies
        ...
        with:
          path: |
            ...
            ~/.gradle/wrapper
          key: ${{ runner.os }}-gradle-${{ hashFiles('gradle/libs.versions.toml', '**/*.gradle*', 'gradle.properties') }}
          restore-keys: |
            ${{ runner.os }}-gradle-

      - name: Validate Gradle Dependency Verification (SHA-256)
        run: ./gradlew :composeApp:desktopTest --no-daemon

      - name: Verify CycloneDX SBOM Generation
        run: node scripts/generate_sbom.cjs

      - name: Check Git Working Directory Cleanliness
        run: git diff --exit-code docs/sbom/

  slsa-attestation:
    name: Generate SLSA Build Provenance (SLSA Level 2+)
    ...
    steps:
      ...
      - name: Build Desktop Artifact
        run: ./gradlew :composeApp:desktopJar --no-daemon

      - name: Attest Desktop Build Provenance (SLSA Level 2+)
        uses: actions/attest-build-provenance@v2
        with:
          subject-path: composeApp/build/libs/*.jar
```

---

### Arquivo: `.github/workflows/firestore-rules.yml`
**Contexto Arquitetural:** Pipeline de validação estrita das regras do Firestore com emulador.

```yaml
name: Firestore Security Rules & Adversarial Suite
...
jobs:
  adversarial-rules-suite:
    name: Adversarial Rules & Isolation Test (Emulator Suite)
    runs-on: ubuntu-latest
    timeout-minutes: 15
    ...
    steps:
      ...
      - name: Setup Java 21 (Required for Firestore Emulator)
        uses: actions/setup-java@v4
        with:
          distribution: 'temurin'
          java-version: '21'
      ...
      - name: Index-as-Code CI Check (Firestore Index Coverage)
        run: |
          npm run check:indexes --prefix functions

      - name: Run Adversarial Rules Suite in Firebase Emulator
        run: |
          npx -y firebase-tools@latest emulators:exec --only firestore "npm --prefix functions run test:emulator"
```

---

### Arquivo: `.github/workflows/ios-build.yml`
**Contexto Arquitetural:** Workflow de verificação de compilação para o ecossistema Apple iOS.

```yaml
name: iOS Simulator Arm64 CI
...
jobs:
  build-ios:
    ...
    steps:
      ...
      - name: Build iOS Simulator Arm64 Binaries
        run: ./gradlew :composeApp:iosSimulatorArm64Binaries --stacktrace
```
