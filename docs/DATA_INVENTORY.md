...
| **Registros de Conexão (MCI Art. 15)** | Firestore (`connectionLogs/{logId}` / `accessLogs/{logId}` / `accessLogsRaw/{logId}`) | **180 dias** (TTL estrito com descarte automático nativo + purga horária pelo Shredder) | **Admin SDK apenas** (read/write: false para clients). **SEM UID, SEM FINGERPRINT, SEM MESSAGEID, SEM PAYLOAD, SEM CHAVE, SEM MNEMÔNICO**. | Art. 7º, II da LGPD c/c Art. 15 da Lei nº 12.965/2014 (MCI) |
...
| **Metadados de Roteamento (`senderId`, `recipientId`, `messageId`)** | Firestore (`messages/{messageId}` e `messages/{messageId}/keys/{keyId}`) e Push Payload (FCM/APNs) | **Máximo de 24 horas** (expurgados atomicamente junto ao envelope). O Push payload inclui o `senderId` (fingerprint) exclusivamente para deep-link, sem conteúdo. | Remetente, Destinatário e Servidor (roteamento, push notification para deep-link, e autorização estrita). | Art. 7º, V — Execução de contrato |
