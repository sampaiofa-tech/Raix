...
**Documento Técnico Formal:** Adequação regulatória e posicionamento legal do ecossistema RAIX (Versão v3.2).
...
*   **Art. 46 e Art. 48 (Segurança, Sigilo e Comunicação de Incidentes):**
    *   *Como o RAIX cumpre:* Arquitetura criptográfica ponta a ponta (E2E) com chaves efêmeras por mensagem e servidor em modelo zero-knowledge. O roteiro criptográfico pós-quântico híbrido (ML-KEM-768 / ML-DSA-65) encontra-se aprovado e implementado, submetendo-se a auditoria externa independente agendada como prioridade máxima da companhia.
...
*   **Art. 15 (Guarda de Registros de Acesso):**
    *   *Como o RAIX cumpre:* O RAIX mantém, em ambiente isolado e de acesso restrito, os registros de acesso a aplicações pelo prazo de 180 dias, em observância ao Art. 15 do Marco Civil da Internet. No estágio atual de implantação, esses registros são conservados sob forma pseudonimizada — resumo criptográfico irreversível do endereço IP (HMAC), com sal rotativo —, acompanhados de carimbo de data/hora e identificação da função acessada. A conservação do registro de acesso em sua forma legal completa — endereço IP e porta lógica de origem — em armazenamento cifrado, isolado e sob acesso controlado e auditável, constitui correção de conformidade já mapeada, dimensionada e priorizada, a ser entregue no próximo ciclo de evolução da plataforma.
...
*   **Art. 22 (Fornecimento de Informações - Ordem Judicial):**
    *   *Como o RAIX cumpre:* A RAIX coopera estritamente com o Poder Judiciário, disponibilizando os registros que efetivamente detém, nos limites da lei. Por desenho, não retém chaves privadas nem conteúdo em claro. Os registros de acesso disponibilizados observam o disposto no Art. 15 da Lei 12.965/2014 e no Decreto 8.771/2016.
...
O ecossistema RAIX encontra-se em conformidade formal em consolidação no território brasileiro, com os itens remanescentes de formalização e a correção de conformidade do Art. 15 já mapeados. Os controles técnicos garantem uma posição forte contra acessos indevidos e provam que o modelo de "privacidade forte por design" não é apenas marketing, mas sim arquitetura executada em código. Jurisdições externas constam em roadmap de maturidade progressiva.
...
---
*Versão v3.2 — incorpora correção de conformidade (Art. 15 do Marco Civil) e ajuste de precisão sobre o estágio de implantação. Parecer técnico: Guru. Aval de conteúdo: Assessor. Data: 2026-09-18.*
